...
def offset(lat: float, lon: float, toEastInMeters: float, toSouthInMeters: float) -> Position:
    AVERAGE_RADIUS_OF_EARTH_M = 6371000
    dLat = -1.0 * (toSouthInMeters / AVERAGE_RADIUS_OF_EARTH_M)
    dLon = toEastInMeters / (AVERAGE_RADIUS_OF_EARTH_M * (math.cos((math.pi * lat) / 180)))
...
def generateCylinderGeojson(trees):
    ...
    for tree in tqdm(trees):
        ...
        topLeft = offset(lat, lng, offsetMeters, -offsetMeters)
        topRight = offset(lat, lng, -offsetMeters, -offsetMeters)
        bottomLeft = offset(lat, lng, offsetMeters, offsetMeters)
        bottomRight = offset(lat, lng, -offsetMeters, offsetMeters)
